...
extern int32_t volatile u32ConversionData_To_HID[96];
int8_t u8ConversionData_To_HID[192];
...
void int32_to_int8()
{
		...
		for(i=0;i<96;i++)
		{
				u8ConversionData_To_HID[i*2]= u32ConversionData_To_HID[i]>>8;
				u8ConversionData_To_HID[i*2+1]= u32ConversionData_To_HID[i];
		}
}
    ...
    if(u32Pages)
    {
        ...
        for(i=0; i<PAGE_SIZE; i++)
            g_u8PageBuff[i] = 0x00;
				for(i=0;i<192;i++)
				{
						g_u8PageBuff[i]=u8ConversionData_To_HID[i];
				}
...
void HID_SetInReport(void)
{
    ...
    if(u8Cmd == HID_CMD_READ)
    {
        ...
        else
        {
            ...
            len = Minimum(g_u32EpAMaxPacketSize, g_u32BytesInPageBuf);
            for (i=0; i<len; i++)
                HSUSBD->EP[EPA].EPDAT_BYTE = g_u8PageBuff[i];//[PAGE_SIZE - g_u32BytesInPageBuf];
            HSUSBD->EP[EPA].EPTXCNT = len;
            HSUSBD_ENABLE_EP_INT(EPA, HSUSBD_EPINTEN_INTKIEN_Msk);
            g_u32BytesInPageBuf -= len;
        }
